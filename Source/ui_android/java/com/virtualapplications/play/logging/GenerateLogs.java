package com.virtualapplications.play.logging;
...
public class GenerateLogs extends AsyncTask<String, Integer, String> {
	...
	protected String doInBackground(String... params) {
		...
		try {
			...
			mLogcatProc = null;
			reader = null;
			int PID = android.os.Process.getUidForName("com.virtualapplications.play");
			mLogcatProc = Runtime.getRuntime().exec(
	...
	protected void onPostExecute(final String response) {
		if (response != null && !response.equals(null)) {
			...
			Toast.makeText(mContext, mContext.getString(R.string.platform),
					Toast.LENGTH_SHORT).show();
			new UploadLogs(mContext, currentTime).execute(response);
		}
	}
}
